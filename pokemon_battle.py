...
class BattleApp:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        root.title("Pokemon Battle - Party Switching (Best of 3)")
        ...
        # Party templates kept constant for the whole match; cloned each round
        ...
        for i, mv in enumerate(self.player.moves):
            btn = tk.Button(btn_frame, text=f"{mv.name}\n({mv.mtype} {mv.power})",
                            command=lambda idx=i: self.on_move_click(idx), width=15, height=3, font=("Arial", 9))
        ...
        self.btn_restart.grid(row=0, column=1, padx=5)

        self.update_record_label()
        # Start first round by restarting the match
        self.restart()
    ...
    def start_new_round(self) -> None:
        # Increment round and reset combatants from templates
        self.round_num += 1
        self.party = [clone_pokemon(t) for t in self.party_templates]
        self.active_idx = 0
        self.player = self.party[self.active_idx]
        # Choose a new enemy (avoid exact name clash if possible)
    ...
    def check_battle_end(self) -> bool:
        ...
        if enemy_fainted and all_party_fainted:
            ...
            # Ties do not affect the match score
            ...
            return False
        # Round ended, moves disabled during transition
        return True

    def _end_round(self, player_won: bool | None) -> None:
        # Update match score on win/loss
        ...
        # Otherwise start next round after a short delay
    ...
    def on_move_click(self, move_idx: int) -> None:
        ...
        if burn_dmg > 0:
            ...
            self.update_display()
            if self.player.hp <= 0 and self.check_battle_end():
                return
    ...
    def on_switch(self) -> None:
        ...
        if not options:
            self.log_message("No healthy Pokémon to switch to!")
            return
        win = tk.Toplevel(self.root)
        win.title("Switch Pokémon")
        win.grab_set()
        tk.Label(win, text="Choose a Pokémon:", font=("Arial", 11, "bold")).pack(pady=8)
        for idx, poke in options:
            txt = f"{poke.name} ({poke.hp}/{poke.max_hp} HP)"
            tk.Button(win, text=txt, width=22, command=lambda i=idx: self._do_switch(win, i)).pack(pady=4)

    def _do_switch(self, dialog: tk.Toplevel, idx: int) -> None:
        dialog.destroy()
        self.active_idx = idx
        self.player = self.party[self.active_idx]
        ...
        def choose(idx: int) -> None:
            ...
            for i, mv in enumerate(self.player.moves):
                if i < len(self.move_buttons):
                    self.move_buttons[i].config(text=f"{mv.name}\n({mv.mtype} {mv.power})",
                                                command=lambda idx=i: self.on_move_click(idx))
            self.lbl_title.config(text=f"Round {self.round_num} - A wild {self.enemy.name} appeared! Go {self.player.name}!")
            self.update_display()
            win.destroy()
    ...
    def restart(self) -> None:
        # Reset match scores and choose a fresh party for the match
        self.player_wins = 0
        self.enemy_wins = 0
        self.round_num = 0
